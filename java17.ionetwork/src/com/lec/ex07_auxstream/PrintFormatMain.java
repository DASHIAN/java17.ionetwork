...
/*
 	printf()메서드
 	PrintStream과 PrintWriter에는 print(), println() 메서드 이외에 printf()메서드가 있다.
 	이 메서드는 형식화된 문자열을 출력할 수 있도록 Java8에서부터 추가된 메서드이다.
 	
 	형식화문자열
 	1. 정수 : %d(정수,123),%6d(6자리정수 왼쪽은 공백 ___123), %-6d(오른쪽공백 123___), %06d(000123)
 	2. 실수 : %10.2(_____123.45), %-10.2f(123.45_____),%010.2f(00000123,45)
 	3. 문자열 : %s(abc),%6s(___abs),%-6s(abc___)
